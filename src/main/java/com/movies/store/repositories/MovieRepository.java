...
public interface MovieRepository extends JpaRepository<Movie, Integer> {
    ...
    @Query(value = "SELECT DISTINCT(m) FROM Movie m JOIN m.category c LEFT JOIN m.information i LEFT JOIN i.actors a " +
            ...
            "(:category IS NULL OR (c.category = :category)) AND (COALESCE(:released) IS NULL OR (i.released = :released)) " +
            "AND (:rating IS NULL OR (i.rating = :rating)) AND " +
            "(COALESCE(:actors, null) IS NULL OR (a IN (SELECT actor FROM Actor actor WHERE actor IN :actors))) AND " +
            "(COALESCE(:directors, null) IS NULL OR (d IN (SELECT director FROM Director director WHERE director IN :directors)))")
